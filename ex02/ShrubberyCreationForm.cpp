...
// 	return ShrubberyCreationFormName;
// }

// void ShrubberyCreationForm::beSigned(const Bureaucrat &bearaucrat) {
// 	if(bearaucrat.getGrade() > AForm::getGradeToSign())
// 		throw GradeTooLowException(bearaucrat.getName());
// 	else
// 		this->AForm::setSign(1);
// }
...
void ShrubberyCreationForm::schrubTree_writer(void) const {
	...
	if(!outfile.is_open())
	{
		...
		// std::cout << "something went wrong while creating a File!\n";
	}
	outfile << "       ^       \n";
    outfile << "      ^^^      \n";
    outfile << "     ^^^^^     \n";
